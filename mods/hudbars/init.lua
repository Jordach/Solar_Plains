...
function hb.register_hudbar(identifier, text_color, label, textures, default_start_value, default_start_max, default_start_hidden, format_string)
	...
	elseif hb.settings.alignment_pattern == "stack_down" then
		...
		if index % 2 == 0 then
			...
			offset_icon = {
				x = hb.settings.start_offset_left.x - 4,
				y = hb.settings.start_offset_left.y - hb.settings.vmargin * (index/2)
			}
		else
			...
			offset_icon = {
				x = hb.settings.start_offset_right.x + 181,
				y = hb.settings.start_offset_right.y - hb.settings.vmargin * ((index-1)/2)
			}
		end
	end
	...
	hudtable.add_all = function(player, hudtable, start_value, start_max, start_hidden)
		...
		if hb.settings.bar_type == "progress_bar" then
			...
			if textures.icon ~= nil then
				ids.icon = player:hud_add({
					...
					text = textures.icon,
					alignment = {x=-1,y=1},
					offset = { x = offset_icon.x, y = offset_icon.y },
				})
			end
		...
		if hb.settings.bar_type == "progress_bar" then
			ids.text = player:hud_add({
				...
				number = text_color,
				direction = 0,
				offset = { x = offset.x + 2,  y = offset.y - 3},
		})
		end
...
function hb.unhide_hudbar(player, identifier)
	...
	local hudtable = hb.get_hudtable(identifier)
	if hudtable == nil then return false end
	if hudtable.hudstate[name].hidden == nil then return false end
...
if minetest.settings:get_bool("enable_damage") or hb.settings.forceload_default_hudbars then
	...
	hb.register_hudbar("breath", 0xFFFFFF, S("Air"), { bar = "hudbars_bar_breath.png", icon = "hudbars_icon_breath.png", bgicon = "hudbars_bgicon_breath.png" }, 10, 10, true)
end
