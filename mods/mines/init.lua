local MINE_DEEP_MIN = -64
local MINE_DEEP_MAX = -30000
local MINE_FACTOR = 1.5
...
local ids = {
	...
	fence = minetest.get_content_id("core:fence_oak"),
	wood = minetest.get_content_id("core:oak_planks"),
	...
	minetest.after(8, function()
		...
		if n ~= nil then
			if n.name == "mines:dummy" then
				minetest.set_node(pos, {name="core:torch",param2=1})
			end
		end
	end)
end
...
local function fill_chest(pos)
	minetest.after(5, function()
		...
		if n ~= nil then
			if n.name == "mines:dummy" then
				minetest.set_node(pos, {name="core:chest"})
