...
function farming.plant_growth_timer(pos, elapsed, node_name)
	...
	if minetest.registered_nodes[stages.stages_left[growth]] then
		local parm2 = minetest.get_node(pos).param2
		minetest.set_node(pos, {name = stages.stages_left[growth], param2 = parm2})
	else
		return true
	end
...
-- notes: croplike is ALWAYS mcore.options("croplike", false, false, false)

function farming.place_seed(itemstack, placer, pointed_thing, plantname, croplike)
	...
	if not minetest.is_protected(pt.above, placer:get_player_name()) then
		...
		if croplike == true then
		
			minetest.set_node(pt.above, {name = plantname, param2 = mcore.options("croplike", false, false, false)})
